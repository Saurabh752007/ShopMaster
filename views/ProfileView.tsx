...
const ProfileView: React.FC = () => {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      // Artificial delay for optimization simulation
      setTimeout(() => {
  ...
  const handleRemoveAvatar = () => {
    const defaultUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${profile.name}`;
    syncAvatar(defaultUrl);
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mb-10 flex flex-col md:flex-row gap-8 items-start md:items-center">
          ...
          <div className={`w-32 h-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden flex items-center justify-center transform transition-all group-hover:rotate-3 relative ${isUploading ? 'bg-sky-50' : 'bg-gray-50'}`}>
            ...
            {isUploading && (
              <div className="absolute inset-0 bg-white/40 backdrop-blur-sm flex items-center justify-center">
                 <p className="text-[10px] font-black text-sky-600 uppercase">Optimizing...</p>
              </div>
            )}
          </div>
          ...
          <div className="absolute -bottom-2 -right-2 flex gap-2">
             ...
             {!avatar.includes('dicebear') && (
               <button 
                 ...
                 className="w-11 h-11 bg-white border border-gray-100 text-red-500 rounded-2xl shadow-lg flex items-center justify-center hover:bg-red-50 hover:border-red-100 transition-all hover:scale-110 active:scale-95"
                 title="Remove Photo"
               >
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden transition-all hover:shadow-xl hover:shadow-gray-100/50">
        <div className="flex border-b border-gray-100 bg-gray-50/50 p-3 gap-3">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-8 animate-in fade-in duration-500">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <InputField label="Owner Name" value={profile.name} onChange={(v) => setProfile({...profile, name: v})} />
               ...
               <div className="flex justify-end">
                  <button onClick={() => showToast('Profile details updated')} className="px-10 py-4 bg-sky-600 text-white font-black rounded-2xl shadow-xl shadow-sky-100 hover:bg-sky-700 active:scale-95 transition-all">
                    Save Changes
                  </button>
               </div>
            </div>
          ...
          {activeTab === 'security' && (
            <div className="space-y-8 animate-in fade-in duration-500">
               <div className="p-8 bg-sky-50 rounded-[2rem] border border-sky-100 flex items-center justify-between">
                  <div>
                    <h3 className="text-xl font-black text-gray-900 tracking-tight">Login Credentials</h3>
                    <p className="text-xs text-sky-600 font-bold uppercase mt-1">Rotate your password regularly</p>
                  </div>
                  <button 
                    onClick={() => setShowPassModal(true)}
                    className="px-6 py-3 bg-white text-gray-900 font-black text-xs uppercase rounded-xl border border-gray-200 hover:bg-gray-900 hover:text-white transition-all shadow-sm"
                  >
                    Change Password
                  </button>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <SecurityToggle 
                    icon="🛡️" 
                    title="Login Alerts" 
                    desc="Email on every new device." 
          ...
          {activeTab === 'preferences' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="p-10 bg-red-50 rounded-[2.5rem] border border-red-100">
                <h3 className="text-xl font-black text-red-900 mb-4">Danger Zone</h3>
                ...
                <button 
                  ...
                  className="px-8 py-3 bg-red-600 text-white font-black text-xs uppercase rounded-xl hover:bg-red-700 transition-all shadow-lg shadow-red-100"
                >
                  Delete Shop Data
                </button>
              </div>
      ...
      {showPassModal && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
             <div className="p-10">
                <h3 className="text-2xl font-black text-gray-900 mb-8">Change Password</h3>
                <form onSubmit={handlePasswordSubmit} className="space-y-6">
                   ...
                   <div className="flex gap-4 pt-4">
                      <button type="button" onClick={() => setShowPassModal(false)} className="flex-1 py-4 font-black text-gray-400">Cancel</button>
                      <button type="submit" className="flex-[2] py-4 bg-sky-600 text-white font-black rounded-2xl shadow-xl shadow-sky-100">Update Password</button>
                   </div>
                </form>
             </div>
          </div>
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
             <div className="p-10 text-center">
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-3">Irreversible Action</h3>
                <p className="text-gray-500 font-medium mb-8">Type <span className="font-black text-red-600 select-none">DELETE DATA</span> to confirm wiping {profile.shopName}.</p>
                <input 
                  type="text" 
                  value={deleteConfirmText}
                  onChange={e => setDeleteConfirmText(e.target.value.toUpperCase())}
                  className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl text-center font-black text-gray-900 outline-none focus:ring-4 focus:ring-red-500/10 focus:border-red-500"
                />
                <div className="flex gap-4 mt-8">
                   <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-4 font-black text-gray-400">Cancel</button>
                   <button 
                     disabled={deleteConfirmText !== 'DELETE DATA'}
                     onClick={() => window.location.reload()}
                     className="flex-[2] py-4 bg-red-600 text-white font-black rounded-2xl shadow-xl shadow-red-100 disabled:opacity-30"
                   >
                     Confirm Wiping
                   </button>
                </div>
...
const InputField: React.FC<{label: string, value: string, type?: string, onChange: (v: string) => void}> = ({ label, value, type = 'text', onChange }) => (
  <div className="space-y-2">
    ...
    <input 
      ...
      className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl outline-none focus:ring-4 focus:ring-sky-500/10 focus:bg-white focus:border-sky-500 font-bold transition-all"
    />
  </div>
);
