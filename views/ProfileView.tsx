...
const ProfileView: React.FC<ProfileViewProps> = ({ onLogout }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mb-10 flex flex-col md:flex-row gap-8 items-center text-center md:text-left">
        <div className="relative group">
          ...
          <input type="file" ref={fileInputRef} className="hidden" accept="image/png, image/jpeg, image/webp" onChange={handleAvatarChange} />
          <div className="absolute -bottom-2 -right-2">
             <button onClick={() => fileInputRef.current?.click()} className="w-11 h-11 bg-gray-900 text-white rounded-2xl shadow-xl flex items-center justify-center hover:bg-sky-600 transition-all hover:scale-110 active:scale-95">
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex border-b border-gray-100 bg-gray-50/50 p-3 gap-2">
          {(['profile', 'security', 'preferences'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-4 px-6 text-xs font-black uppercase tracking-widest rounded-2xl transition-all ${
                activeTab === tab ? 'bg-white text-sky-600 shadow-xl border border-sky-50' : 'text-gray-400'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="p-8 md:p-10">
          {activeTab === 'profile' && (
            <div className="space-y-8">
               ...
               <button onClick={handleSaveProfile} disabled={isSaving} className="w-full py-5 bg-sky-600 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-2">
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
               <SecurityToggle icon="📱" title="Two-Factor Verification" desc={`Linked to ${profile.phone}`} enabled={profile.twoFactor} onToggle={() => setProfile({...profile, twoFactor: !profile.twoFactor})} />
               <SecurityToggle icon="🛡️" title="Security Alerts" desc={`Sent to ${profile.email}`} enabled={profile.notifications} onToggle={() => setProfile({...profile, notifications: !profile.notifications})} />
            </div>
          )}

          {activeTab === 'preferences' && (
            <div className="space-y-8">
                <div className="p-8 bg-gray-50 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                  ...
                  <button onClick={onLogout} className="w-full md:w-auto px-10 py-5 bg-gray-900 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-3">
                ...
                <div className="p-8 bg-red-50 rounded-[2.5rem] border border-red-100">
                  ...
                  <p className="text-sm text-red-700/70 mb-8">Permanently delete everything and start fresh.</p>
                  <button onClick={() => setShowDeleteModal(true)} className="w-full md:w-auto px-8 py-4 bg-red-600 text-white font-black text-xs uppercase rounded-xl">
                    Reset All Data
                  </button>
                </div>
            </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-10 text-center">
             <div className="w-20 h-20 bg-red-50 text-red-600 rounded-3xl flex items-center justify-center text-4xl mx-auto mb-6">🗑️</div>
             <h3 className="text-2xl font-black text-gray-900 mb-3 tracking-tight">Confirm Reset</h3>
             <p className="text-gray-500 font-medium mb-8">Type <span className="font-black text-red-600">DELETE DATA</span> to wipe all records.</p>
             <input type="text" value={deleteConfirmText} onChange={e => setDeleteConfirmText(e.target.value.toUpperCase())} className="w-full px-6 py-5 bg-gray-50 border-transparent rounded-2xl text-center font-black outline-none focus:ring-4 focus:ring-red-500/10" placeholder="Verification..." />
             <div className="flex flex-col gap-4 mt-8">
                <button disabled={deleteConfirmText !== 'DELETE DATA'} onClick={handleFactoryReset} className="w-full py-5 bg-red-600 text-white font-black rounded-2xl shadow-xl disabled:opacity-30">
                  Confirm Factory Reset
                </button>
                <button onClick={() => setShowDeleteModal(false)} className="py-2 font-black text-gray-400">Cancel</button>
             </div>
          </div>
        </div>
      )}
...
const InputField: React.FC<{label: string, value: string, type?: string, onChange: (v: string) => void}> = ({ label, value, type = 'text', onChange }) => (
  <div className="space-y-2 text-left">
    ...
    <input type={type} value={value} onChange={e => onChange(e.target.value)} className="w-full px-6 py-4 bg-gray-50 border-transparent rounded-2xl outline-none focus:ring-4 focus:ring-sky-500/10 focus:bg-white font-bold transition-all" />
  </div>
);
...
const SecurityToggle: React.FC<{icon: string, title: string, desc: string, enabled: boolean, onToggle: () => void}> = ({ icon, title, desc, enabled, onToggle }) => (
  <div onClick={onToggle} className="flex items-center justify-between p-6 border border-gray-100 rounded-3xl bg-white hover:border-sky-500 transition-all cursor-pointer group">
